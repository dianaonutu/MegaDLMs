...
def _pretrain(
    ...
    if args.virtual_pipeline_model_parallel_size is not None:
        ...
        print_rank_0(
            "benchmark eval iterator is not yet supported for virtual pipeline model parallel"
        )
    else:
    ...
    if args.debug:
        ...
        with torch.autograd.detect_anomaly() if args.detect_anomaly else nullcontext():
            if args.detect_anomaly:
                print(
                    "WARNING: Detecting anomaly is turned on. This will slow down the training process."
                )
            return _pretrain(
                train_valid_test_dataset_provider,
                model_provider,
...
def setup_model_and_optimizer(
    ...
    if (
        ...
        checkpoint_load_strict = (
            False
            if "embedding_hidden_fusion_type" in args
            and args.embedding_hidden_fusion_type == "weight_fusion"
            else True
        )
        if not checkpoint_load_strict:
            print_rank_0(
                "WARNING: Loading checkpoint with non-strict mode. This is a workaround only for the plmt weight fusion mode."
            )
...
def setup_model_only(model_provider_func, model_type, checkpointing_context=None):
    """Setup model and optimizer."""
    args = get_args()
    timers = get_timers()
    one_logger = get_one_logger()
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config):
    ...
    if should_exit:
        ...
        print_rank_0(
            f" > Rerun state machine detected an error. "
...
def training_log(
    ...
    if writer and (iteration % args.tensorboard_log_interval == 0):
        ...
        if total_loss_dict.get(skipped_iters_key, 0) > 0:
            writer.add_scalar(
                'skipped-iterations', total_loss_dict.get(skipped_iters_key, 0), iteration
            )
            if wandb_writer:
                wandb_writer.log(
                    {'skipped-iterations': total_loss_dict.get(skipped_iters_key, 0)}, iteration
        ...
        if ft_client is not None:
            ...
            if ft_integration.can_update_timeouts():
                ft_integration.get_rank_monitor_client(
                    ft_integration.StateMachineActions.UPDATE_TIMEOUT
                ).calculate_and_set_timeouts()
                print_rank_0(
                    f'Updated FT timeouts. New values: \
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        if (
            ...
            if args.enable_ft_package and ft_integration.get_rank_monitor_client() is not None:
                ft_integration.get_rank_monitor_client(
                    ft_integration.StateMachineActions.EVAL_HEARTBEAT
                ).send_heartbeat()
...
def evaluate_with_fixed_corruption(
    ...
    num_splits = (
        args.num_latent_validation_splits if hasattr(args, 'num_latent_validation_splits') else 5
    )
    p_values = [
        args.min_p + i * (args.max_p - args.min_p) / (num_splits - 1) for i in range(num_splits)
    ]
    l_values = [
        int(args.min_l + i * (args.max_l - args.min_l) / (num_splits - 1))
...
def get_train_valid_test_num_samples():
    ...
    if (
        "num_latent_validation_splits" in args
        and "use_latent_validation" in args
        and args.use_latent_validation
    ):
        num_latent_validation_splits = (
            args.num_latent_validation_splits + 1
            if 'add_vanilla_ntp_validation' in args and args.add_vanilla_ntp_validation
            else args.num_latent_validation_splits
        )

        eval_iters *= num_latent_validation_splits
        test_iters *= num_latent_validation_splits
        print(
            "WARNING: Using latent validation, eval_iters and test_iters are multiplied by num_latent_validation_splits. If you are not using pretrain_plmt.py, please check the corresponding arguments 'use_latent_validation' and 'num_latent_validation_splits' to turn it off."
        )

    # only for pretrain_difflm.py
    if "use_difflm_validation" in args and args.use_difflm_validation:
        eval_iters *= 2
        test_iters *= 2
        print(
            "WARNING: Using difflm validation, eval_iters and test_iters are multiplied by 2 to include the normal NWP validation. If you are not using pretrain_difflm.py, please check the corresponding arguments 'use_difflm_validation' to turn it off."
        )
